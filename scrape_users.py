...
LOGIN_URL = 'http://nomadlist.com/userApi.php?action=login_by_email&hash=8a50429958a8730e6ff97e65e1c48407ff3e065e'
...
def get_users_info(d, usernames):  # TODO: fix bug with trips info
    """
    ...
    for u in usernames:
        ...
        trip_ids = []
        for t in trips:
            trip_ids.append(t.get_attribute('id'))
        ...
        for tid in trip_ids:
            ...
            for e in TRIP_ELEMENTS:
                if e == 'name':
                    trips_dict[tid][e] = d.find_element_by_xpath(
                        f"//tr[@id='{tid}']//td[contains(@class,'{e}')]//h2").text
                else:
                    trips_dict[tid][e] = d.find_element_by_xpath(
                        f"//tr[@id='{tid}']//td[contains(@class,'{e}')]").text
        res[u]['trip_list'] = trips_dict

        # res[u]['twitter'] = d.find_element_by_xpath(
        #     "//a[contains(@class,'action-contact-user-twitter')]").get_attribute('href').split('/')[-1]
        # res[u]['instagram'] = d.find_element_by_xpath(
        #     "//a[contains(@class,'action-contact-user-instagram')]").get_attribute('href').split('/')[-1]
...
def main():
    ...
    users_dict = get_users_info(driver, users_list[:3])
