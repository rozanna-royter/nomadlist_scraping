...
def get_users_info(d, usernames):
    ...
    for u in usernames:
        ...
        res[u]['trip_list'] = get_trips(soup_html)
...
def get_trips(soup):
    """Returns trips info from current page"""
...
def get_socials(d, sn_name):
    ...
    try:
        return d.find_element_by_xpath(
            f"//a[contains(@class,'action-contact-user-{sn_name}')]").get_attribute('href').split('/')[-1]
...
def extraction_cycle(driver, users_list):
    ...
    if not START_FROM_TOP:
        users_dict = read_dict_from_json(USERS_INFO_FILENAME)
